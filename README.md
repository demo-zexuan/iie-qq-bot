...
- `WANGDAO_ENABLED` 是否启用王道关键词随机图片插件（默认 `false`）
- `WANGDAO_GROUP_WHITELIST` 生效群号白名单（逗号分隔，留空表示所有群）
- `WANGDAO_KEYWORDS` 触发关键词（逗号分隔，默认 `王道`）
- `WANGDAO_CASE_SENSITIVE` 关键词匹配是否区分大小写（默认 `false`）
- `WANGDAO_IMAGE_DIR` 图片目录（默认 `wangdao`，支持绝对路径）
- `WANGDAO_MAX_TRIGGERS_PER_WINDOW` 滚动窗口内最多触发次数（默认 `1`）
- `WANGDAO_WINDOW_SECONDS` 滚动窗口时长（秒，默认 `300`）
- `WANGDAO_RATE_LIMIT_BY_GROUP` 按群覆盖限流配置（格式 `群号:次数:秒数,群号:次数:秒数`）
...
WANGDAO_ENABLED=true
WANGDAO_GROUP_WHITELIST=12345678,87654321
WANGDAO_KEYWORDS=王道,408王道
WANGDAO_CASE_SENSITIVE=false
WANGDAO_IMAGE_DIR=wangdao
WANGDAO_MAX_TRIGGERS_PER_WINDOW=1
WANGDAO_WINDOW_SECONDS=300
# 按群覆盖：群12345678每5分钟最多1次，群87654321每1分钟最多3次
WANGDAO_RATE_LIMIT_BY_GROUP=12345678:1:300,87654321:3:60
